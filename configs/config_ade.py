...
"""Dataset Config"""
C.DATASET = edict()
C.DATASET.name = 'ade20k'
C.DATASET.root = osp.join(C.SYSTEM.root_dir, 'data/ade20k')
C.DATASET.mode = 'RGB'
...
C.DATASET.num_train_imgs = 25574
C.DATASET.num_classes = 150  #for cityscape
C.DATASET.norm_mean = np.array([.485, .456, .406]) # For ADE
C.DATASET.norm_std = np.array([.229, .224, .225])
C.DATASET.base_size = 520
C.DATASET.crop_size = 512
...
""" Dataset --> Darken Augmentation. Imp for CityScape """
C.DATASET.DARKEN = edict()
C.DATASET.DARKEN.darken = True
...
"""Image Config"""
C.IMAGE = edict()
C.IMAGE.background = -1
C.IMAGE.image_height = 512
C.IMAGE.image_width = 512
