...
class PyramidVisionTransformer(nn.Module):
    ...
    def _get_pos_embed(self, pos_embed, patch_embed, H, W):
        ...
        if H * W == self.patch_embed1.num_patches:
            ...
            print(f'pos embed interpolation:{pos_embed.shape}')
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            x, (H, W) = patch_embed(x)
            print(f'patch embed x:{x.shape} H:{H} W:{W} pe H:{patch_embed.H} pe W:{patch_embed.W}')
            print(f'pos_embed created: {pos_embed.shape}')
            if i == self.num_stages - 1:
                ...
                print(f'pos embed finalized: {pos_embed.shape}')
                
                # exit()


            x = pos_drop(x + pos_embed)
            print(f'after pos embed x: {x.shape}')
            count = 1
            for blk in block:
                print(f'####### block:{count} #######')
                x = blk(x, H, W)
