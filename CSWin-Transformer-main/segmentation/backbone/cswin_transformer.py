...
class LePEAttention(nn.Module):
    ...
    def forward(self, temp):
        ...
        print(f'after patched q:{q.shape} k:{k.shape} v:{v.shape}')
...
class CSWinBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.branch_num == 2:
            print(f'temp divided: {temp[:,:,:C//2,:,:].shape}')
...
def img2windows(img, H_sp, W_sp):
    ...
    print(f'img_reshape:{img_reshape.shape} H_sp:{H_sp} W_sp:{W_sp}')
...
class CSWin(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=64, depth=[1,1,1,1], split_size = 7,
                 num_heads=[2,2,4,8], mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
