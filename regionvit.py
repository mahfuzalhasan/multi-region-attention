...
def convert_to_flatten_layout(cls_tokens, patch_tokens, ws):
    ...
    if need_mask:
        ...
        mask[-1, ...] = bottom_right
        print(f'final mask:{mask}, shape:{mask.shape}')
    ...
    print(f'final mask shape:{mask.shape} patch_tokens:{patch_tokens.shape}')
    exit()
...
class RegionViT(nn.Module):
    ...
    def forward_features(self, x, detection=False):
        ...
        print(f'patch embed:{x.shape} cls_token:{cls_tokens.shape}')
